...
export const grid: { [index: number]: GridBlock } = {};
  ...
  for (const pieceName in pieces) {
    ...
    switch (side) {
      ...
      case PlayerSide.left:
        config.pos = rotate(pieces[pieceName].pos, 1, true);
        break;
      case PlayerSide.bottom:
        config.pos = rotate(pieces[pieceName].pos, 2, true);
        break;
      case PlayerSide.right:
        config.pos = rotate(pieces[pieceName].pos, 3, true);
        break;
    }
...
function createNewPlayerMesh(player: Player, assets: InstantiatedEntries) {
  ...
  for (const pieceName in player.pieces) {
    ...
    meshes[pieceName] = configureMesh(
      assets.rootNodes[0].getChildMeshes()
